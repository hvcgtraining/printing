...
!function ($) {
  ...
  function Multiselect(select, options) {
    ...
    this.$select = $(select); // Initialization.
    ...
    this.options.onDropdownHide = $.proxy(this.options.onDropdownHide, this); // Build select all if enabled.
  ...
  Multiselect.prototype = {
    ...
    buildButton: function buildButton() {
      this.$button = $(this.templates.button).addClass(this.options.buttonClass); // Adopt active state.
    ...
    buildDropdownOptions: function buildDropdownOptions() {
      ...
      $('li input', this.$ul).on('change', $.proxy(function (event) {
        ...
        var isSelectAllOption = $(event.target).val() === this.options.selectAllValue; // Apply or unapply the configured selected class.
        ...
        if (event.shiftKey) {
          ...
          if (checked) {
            ...
            if (currentIdx > prevIdx) {
              $(event.target).parents("li:last").prevUntil(prev).each(function () {
                $(this).find("input:first").prop("checked", true).trigger("change");
              });
            } else {
              $(event.target).parents("li:last").nextUntil(prev).each(function () {
                $(this).find("input:first").prop("checked", true).trigger("change");
              });
            }
          }
    ...
    createOptgroup: function createOptgroup(group) {
      ...
      this.$ul.append($li); // Add the options of the group.
    ...
    buildSelectAll: function buildSelectAll() {
      var alreadyHasSelectAll = this.hasSelectAll(); // If options.includeSelectAllOption === true, add the include all checkbox.
     ...
     */
    select: function select(selectValues) {
     ...
     */
    deselect: function deselect(deselectValues) {
    ...
    rebuild: function rebuild() {
      this.$ul.html(''); // Remove select all option in select.

      $('option[value="' + this.options.selectAllValue + '"]', this.$select).remove(); // Important to distinguish between radios and checkboxes.
    ...
    dataprovider: function dataprovider(_dataprovider) {
      ...
      _dataprovider.forEach(function (option) {
    ...
    updateButtonText: function updateButtonText() {
      ...
      $('button', this.$container).html(this.options.buttonText(options, this.$select)); // Now update the title attribute of the button.
