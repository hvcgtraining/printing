...
!function ($) {
  var DateRangePicker = function DateRangePicker(element, options, cb) {
    ...
    this.element = $(element); //create the picker HTML object

    var DRPTemplate = '<div class="daterangepicker">' + '<div class="calendars">' + '<div class="calendar left"></div>' + '<div class="calendar right"></div>' + '</div>' + '<div class="ranges">' + '<div class="range_inputs">' + '<div class="daterangepicker_start_input">' + '<label for="daterangepicker_start"></label>' + '<input class="form-control" type="text" name="daterangepicker_start" value="" disabled="disabled">' + '<button class="applyBtn" disabled="disabled"></button>' + '</div>' + '<div class="daterangepicker_end_input">' + '<label for="daterangepicker_end"></label>' + '<input class="form-control" type="text" name="daterangepicker_end" value="" disabled="disabled">' + '<button class="cancelBtn"></button>' + '</div>' + '</div>' + '</div>' + '</div>';
    this.parentEl = _typeof(options) == 'object' && options.parentEl && $(options.parentEl) || $(this.parentEl);
    this.container = $(DRPTemplate).appendTo(this.parentEl); //custom options

    if (_typeof(options) != 'object') options = {};
    this.setOptions(options, cb); //apply CSS classes and labels to buttons
  ...
  DateRangePicker.prototype = {
    ...
    setOptions: function setOptions(options, callback) {
      ...
      if (_typeof(options.dateLimit) == 'object') this.dateLimit = options.dateLimit; // update day names order to firstDay
      ...
      if (_typeof(options.ranges) == 'object') {
        for (var range in options.ranges) {
          ...
          var end = moment(options.ranges[range][1]); // If we have a min/max date set, bound this range
    ...
    enterDate: function enterDate(e) {
    ...
    updateMonthYear: function updateMonthYear(e) {
      ...
      if (!isLeft) cal = this.container.find('.calendar.right'); // Month must be Number for new moment versions
    ...
    buildCalendar: function buildCalendar(month, year, hour, minute, side) {
    ...
    renderCalendar: function renderCalendar(calendar, selected, minDate, maxDate) {
